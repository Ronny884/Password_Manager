...
class NewDatabaseWindowFunctionality(QMainWindow, FileManager):
    """
    ...
    """
    def __init__(self):
        super(NewDatabaseWindowFunctionality, self).__init__()
        self.window__1 = None
        self.window__G = None
        self.window__7 = None
        self.window__9 = None
        self.tuple_with_need_name_and_password = False, None, None
    ...
    def set_default_state(self):
        ...
        '''
        self.is_db = False
        self.set_empty_info_labels()
        self.customize_eye_button('icons/eye_2.png')
    ...
    def set_empty_info_labels(self):
        self.window_5.label_name_info.setText('')
        self.window_5.label_password_info.setText('')
    ...
    def tracking_db_name(self):
        ...
        if len(new_db_name_now) > 2 and invalid_chars_signal is True:
            self.general_state_db_name = True
            self.set_border_color(self.window_5.enter_db_name, '(0,160,0)')
            self.window_5.label_name_info.setText('Valid name')
            ...
            self.general_state_db_name = False
            if len(new_db_name_now) < 3:
                self.window_5.label_name_info.setText('The name must contain at least 3 characters')
            elif invalid_chars_signal is False:
                self.window_5.label_name_info.setText('The name must not contain some special characters or spaces')
    ...
    def tracking_master_password(self):
        ...
        '''
        valid_folder_chars_master_password = ascii_lowercase + ascii_uppercase + digits
        invalid_chars_signal = None
        master_password_now = self.window_5.enter_master_password.text()
        for i in master_password_now:
            if i in valid_folder_chars_master_password:
                invalid_chars_signal = True
            else:
                invalid_chars_signal = False
                break

        if len(master_password_now) > 8 and invalid_chars_signal is True:
            self.general_state_master_password = True
            self.set_border_color(self.window_5.enter_master_password, '(0,160,0)')
            self.window_5.label_password_info.setText('Valid password')
        elif len(master_password_now) == 0:
            self.general_state_master_password = False
            self.set_border_color(self.window_5.enter_master_password, '(244, 244, 244)')
            self.window_5.label_password_info.setText('')
        else:
            if len(master_password_now) < 9:
                self.window_5.label_password_info.setText('Password must contain at least 9 characters')
            elif invalid_chars_signal is False:
                self.window_5.label_password_info.setText('Password must not contain spaces, special characters, non-Latin letters')
            self.general_state_master_password = False
    ...
    def btn_create_state(self):
        ...
        '''
        if self.general_state_master_password is True and self.general_state_db_name is True:
            self.window_5.btn_create.show()
        else:
            self.window_5.btn_create.hide()

    def clean_db_name_field(self):
        self.window_5.enter_db_name.setText('')
    ...
    def create_db(self):
        ...
        if self.checking_for_file_existence('Database', f'{self.finish_new_db_name}.db'):
            ...
            self.window_6_incorrect_name.buttonClicked.connect(self.clean_db_name_field)
            self.window_6_incorrect_name.exec()
        else:
            # self.db.create_new_db(self.finish_new_db_name)
            path = f'{os.getcwd()}/Database/{self.finish_new_db_name}.db'
            self.db.create_new_db(path)
            self.tuple_with_need_name_and_password = (True, self.finish_new_db_name, self.finish_master_password)
            self.is_db = True
            self.window_5.btn_create.hide()
            self.set_empty_info_labels()
    ...
    def create_new_note(self):
        ...
        self.window__7.tuple_with_need_name_and_password = (True, self.finish_new_db_name, self.finish_master_password)
    ...
    def again(self):
        if self.window__1.road_map_list[-1] == 1:
            if self.is_db:
    ...
    def customize_eye_button(self, icon_file):
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(icon_file), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        self.window_5.btn_hide_show.setIcon(icon)
    ...
    def hide_or_show_password(self):
        ...
        if self.window_5.enter_master_password.echoMode() is QLineEdit.EchoMode.Normal:
            ...
            self.customize_eye_button('icons/eye.png')
        else:
            self.window_5.enter_master_password.setEchoMode(QLineEdit.EchoMode.Normal)
            self.customize_eye_button('icons/eye_2.png')
...
"}")
        ...
        self.label_name_info = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_name_info.setGeometry(QtCore.QRect(180, 130, 591, 31))
        self.label_name_info.setMinimumSize(QtCore.QSize(100, 0))
        self.label_name_info.setStyleSheet("background-color: rgba(0, 0, 0, 0);\n"
                                      "color: rgb(72,72,72);\n"
                                      "\n"
                                      "font: \"MS Shell Dlg 2\";\n"
                                      "font-size: 16pt;\n"
                                      ...
                                      "\n"
                                      "")
        self.label_name_info.setObjectName("label_info")
        self.label_password_info = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_password_info.setGeometry(QtCore.QRect(180, 295, 700, 31))
        self.label_password_info.setMinimumSize(QtCore.QSize(100, 0))
        self.label_password_info.setStyleSheet("background-color: rgba(0, 0, 0, 0);\n"
                                           "color: rgb(72,72,72);\n"
                                           "\n"
                                           "font: \"MS Shell Dlg 2\";\n"
                                           "font-size: 16pt;\n"
                                           ...
                                           "\n"
                                           "")
        self.label_password_info.setObjectName("label_info")
