...
class MyEventHandler(FileSystemEventHandler, FileManager):
    """
...
class DatabaseSelectionWindowFunctionality(QMainWindow, FileManager):
    def __init__(self):
        super(DatabaseSelectionWindowFunctionality, self).__init__()
    ...
    def set_default_state(self):
        ...
        if self.checking_for_folder_existence('Database'):
            self.window_3.listWidget.setFont(QFont("MS Shell Dlg 2", 20))
            self.window_3.listWidget.clear()
            for file_name in self.get_listdir('Database'):
                if file_name.endswith('.db'):
                    item = QListWidgetItem('  ' + file_name.replace('.db', ''))

                    if self.checking_for_file_existence('Alpha', f'{file_name.replace(".db", ".bin")}'):
