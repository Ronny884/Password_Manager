...
class LookNoteWindowFunctionality(QMainWindow):
    def __init__(self):
        super(LookNoteWindowFunctionality, self).__init__()
    ...
    def set_default_state(self):
        try:
            ...
            if self.window__1 is None or self.window__1.road_map_list[-1] == 9:
                ...
                path_for_id = f'{os.getcwd()}/Database/{self.window__9.working_db_name}.db'
                self.main_note_name = self.db.get_name_by_id(path_for_id,
    ...
    def copy_username(self):
        pyperclip.copy(self.db.select_parameter_for_copy(self.path, self.main_note_name, 'username'))
        self.window__9.copy_information('Copied', self.window_10.btn_copy_username)

    def copy_password(self):
        pyperclip.copy(self.db.select_parameter_for_copy(self.path, self.main_note_name, 'password'))
        self.window__9.copy_information('Copied', self.window_10.btn_copy_password)

    def copy_login(self):
        pyperclip.copy(self.db.select_parameter_for_copy(self.path, self.main_note_name, 'login'))
        self.window__9.copy_information('Copied', self.window_10.btn_copy_login)

    def copy_passphrase(self):
        pyperclip.copy(self.db.select_parameter_for_copy(self.path, self.main_note_name, 'passphrase'))
        self.window__9.copy_information('Copied', self.window_10.btn_copy_passphrase)

    def copy_phone_number(self):
        pyperclip.copy(self.db.select_parameter_for_copy(self.path, self.main_note_name, 'phone_number'))
        self.window__9.copy_information('Copied', self.window_10.btn_copy_phone_number)

    def copy_url(self):
        pyperclip.copy(self.db.select_parameter_for_copy(self.path, self.main_note_name, 'url'))
    ...
    def push_ok(self, btn):
        if btn.text() == 'OK':
            path = f'{os.getcwd()}/Database/{self.window__9.working_db_name}.db'
            self.db.delete_note(path, self.main_note_name)
            self.window__1.road_map_list.append(10)
            self.tuple_with_need_name_and_password = False, None, None
            self.window__9.set_default_state()
            self.window__9.show()
