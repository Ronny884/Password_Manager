...
from PyQt6.QtWidgets import QMainWindow, QMessageBox, QToolTip, QListWidgetItem
import pyperclip
from data_manager import DataManager
from encryption_manager import EncryptionManager


class DatabaseInteriorWindowFunctionality(QMainWindow):
    def __init__(self):
        super(DatabaseInteriorWindowFunctionality, self).__init__()
        ...
        self.window_9.notes_list.itemDoubleClicked.connect(self.handle_note_item_double_click)
    ...
    def set_default_state(self):
        ...
        if self.working_db_name is not None:
            self.path = f'{os.getcwd()}/Database/{self.working_db_name}.db'
            try:
                self.filling_in_the_list_of_notes(None)
                path = f'{os.getcwd()}/Database/{self.working_db_name}.db'
                self.category_list = list(set(self.db.select_all_categories(path)))
    ...
    def showEvent(self, event):
        self.tuple_with_need_name_and_password = True, self.working_db_name, self.password_for_encryption
    ...
    def filling_in_the_list_of_notes(self, category_parametr):
        ...
        path = f'{os.getcwd()}/Database/{self.working_db_name}.db'
        self.note_name_list = self.db.select_notes_with_or_without_category(path, category_parametr)
    ...
    def handle_note_item_click(self, item, name=None):
        try:
            ...
            self.chosen_note_id = self.db.select_parameter_for_copy(self.path, self.chosen_note_name, 'id')
    ...
    def handle_note_item_double_click(self):
        self.open_a_note()
    ...
    def copy_password(self):
        try:
            pyperclip.copy(self.db.select_parameter_for_copy(self.path, self.chosen_note_name, 'password'))
    ...
    def copy_login(self):
        try:
            login = self.db.select_parameter_for_copy(self.path, self.chosen_note_name, 'login')
            if login == '':
